...
function commandStats(msg) {
  ...
  client.voiceConnections.map(vc => vc.channel)
    .forEach(c => users += c.members // eslint-disable-line
      .filter(m => !m.selfDeaf && !m.deaf).size - 1); // eslint-disable-line id-length
...
function currentListeners() {
  ...
  client.voiceConnections
    .map(vc => vc.channel)
    .forEach(c => userCount += c.members // eslint-disable-line
      .filter(m => !m.selfDeaf && !m.deaf).size - 1); // eslint-disable-line id-length
